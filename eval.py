...
def run_evaluation():
    ...
    # Generate answers and collect metadata
    ...
    # ----------------------------------------------------------------
    print("\n📊 Computing Lexical and Semantic Scores separately...")

    report = {}
    for lang in ["English", "Amharic"]:
        lang_df = df[df["language"] == lang]

        bleu_score = bleu.compute(
            predictions=lang_df["answer"].tolist(),
            references=[[gt] for gt in lang_df["ground_truth"].tolist()]
        )
        bert_res = bertscore.compute(
            predictions=lang_df["answer"].tolist(),
            references=lang_df["ground_truth"].tolist(),
        ...
        report[lang] = {
            ...
            "BERTScore": avg_bert,
            "Avg Latency": lang_df["latency"].mean(),
            "Total Queries": len(lang_df)
        }
    ...
    for lang, metrics in report.items():
        ...
        print(f"   Average Latency: {metrics['Avg Latency']:.2f}s")
