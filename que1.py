...
# Swapping Google Embeddings for Hugging Face
...
def generate_answer(user_query, language="English"):
    try:
        # 2. Setup Multilingual Embeddings (Matching ingest.py)
        # This allows Amharic voice queries to find English medical chunks
        ...
        # Load DB with the new embedding function
        ...
        for doc in docs:
            ...
            # Get source from metadata safely
            source_path = doc.metadata.get('source', 'Unknown')
            source_name = os.path.basename(source_path)
            sources.add(source_name)

        # 5. Initialize LLM: Gemini 2.5 Flash
        # Keeping Gemini as the 'Brain' because of its excellent bilingual reasoning
        llm = ChatGoogleGenerativeAI(
            model="gemini-2.5-flash", 
            temperature=0.1, 
        )
        
        # 6. Optimized Prompt for Bimodal Output
        ...
        # 7. Execution with Exponential Backoff
        for attempt in range(2): 
            try:
                ...
                # Clean up response content for gTTS safety
