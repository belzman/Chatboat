...
try:
    ...
    print("❌ Error: 'que.py' not found. Ensure the file exists to run evaluation.")
...
def run_evaluation():
    ...
    print("\n📊 Computing Lexical and Semantic Scores...")
    ...
    threshold = 0.8
    binary_preds = [1 if f >= threshold else 0 for f in bert_res["f1"]]
    binary_refs = [1] * len(binary_preds)  # all ground truths are "correct"
    acc = accuracy_score(binary_refs, binary_preds)
    prec = precision_score(binary_refs, binary_preds, zero_division=0)
    rec = recall_score(binary_refs, binary_preds, zero_division=0)
    f1 = f1_score(binary_refs, binary_preds, zero_division=0)
    roc_auc = roc_auc_score(binary_refs, bert_res["f1"])
    RocCurveDisplay.from_predictions(binary_refs, bert_res["f1"])
    plt.title("ROC Curve (Semantic Correctness via BERTScore)")
    plt.savefig("roc_curve.png")
    plt.close()
    print("\n" + "=" * 70)
    print("📈 SMART HIV CARE: BILINGUAL EVALUATION REPORT")
    ...
    print(f"Overall BERTScore: {avg_bert:.4f}")
    ...
    print("💾 Detailed report saved to 'smarthivcare_eval_final.csv'")
